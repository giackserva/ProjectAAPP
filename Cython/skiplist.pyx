...
cdef class SkipList:
    ...
    cpdef int _random_level(self):
        cdef int level = 0
        cdef double random = rand() / RAND_MAX
        # int(rand()/(RAND_MAX100.0)) # random int 1..100
    ...
    cpdef bint insert(self, int key, int value):
        ...
        if key <= INT_MIN or key >= self.INT_MAX:
